...
void Parser::Bsm() {
		bsm = std::make_shared<class Bsm>(); 
		unsigned curline = la->line, curcol = la->col; 
		HexLitPtr hexlit; 
		while (la->kind == _customTokenHexLit) {
			HexLit(hexlit);
			bsm->hexlits.push_back(hexlit); 
		}
...
void Errors::SynErr(int line, int col, int n) {
	...
	switch (n) {
			...
			case 1: s = coco_string_create(L"customTokenHexLit expected"); break;
			case 2: s = coco_string_create(L"??? expected"); break;
