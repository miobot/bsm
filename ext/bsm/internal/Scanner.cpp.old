...
void Scanner::Init() {
	...
	for (i = 97; i <= 102; ++i) start.set(i, 1);
	start.set(36, 5);
...
Token* Scanner::NextToken() {
	...
	switch (state) {
		...
		case 1:
			if ((ch >= L'0' && ch <= L'9') || (ch >= L'a' && ch <= L'f')) {AddCh(); goto case_2;}
			else {goto case_0;}
		case 2:
			case_2:
			{t->kind = 1; break;}
		case 3:
			...
			if ((ch >= L'0' && ch <= L'9') || (ch >= L'a' && ch <= L'z')) {AddCh(); goto case_3;}
			else {t->kind = 3; break;}
		case 4:
			case_4:
			recEnd = pos; recKind = 4;
			if ((ch >= L'-' && ch <= L'.') || (ch >= L'0' && ch <= L':') || (ch >= L'a' && ch <= L'z')) {AddCh(); goto case_4;}
			else {t->kind = 4; break;}
		case 5:
			recEnd = pos; recKind = 3;
			if ((ch >= L'0' && ch <= L'9')) {AddCh(); goto case_3;}
			else if ((ch >= L'a' && ch <= L'z')) {AddCh(); goto case_6;}
			else {t->kind = 3; break;}
		case 6:
			case_6:
			recEnd = pos; recKind = 3;
